...
然后我们就是安装ndctl，在安装之前我们首先需要进行一些依赖库的下载，有可能他会提示你有一些archives无法得到，不过这并不重要。
	...
	sudo apt-get install autoconf asciidoc xmlto automake libtool systemd pkg-config 
...
<center>![在这里插入图片描述](https://img-blog.csdnimg.cn/20190426100458175.PNG)</center>
...
<center>![在这里插入图片描述]("https://img-blog.csdnimg.cn/20190426100817848.png?x-oss-process=image/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L3dlaXhpbl8zOTgyMDc5Mw==,size_16,color_FFFFFF,t_70")</center>

这个时候我们需要进入/usr/local/lib中，将libuuid.so.1删除掉，然后再进行make即可。

<center>![在这里插入图片描述]("https://img-blog.csdnimg.cn/20190426101625244.PNG")</center>

然后我们执行make check检测时候安装成功。
